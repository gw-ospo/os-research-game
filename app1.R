...
generate_bin_js <- function(bins) {
  bin_code <- paste(sapply(bins, function(bin) {
    sprintf("
          ...
          console.log('%s items:', %sItems);",
            bin$id, bin$id, bin$id, bin$id, bin$id, bin$id, bin$id)
  }), collapse = "\n")
  return(bin_code)
}
...
ui <- fluidPage(
  ...
  div(class = "title-container", style = "padding: 15px 15px 0 15px;",
      h2(class = "main-title", app_config$title, style = "display: inline-block; margin: 0;"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit, {
    ...
    unknown_bin_id <- if (!is.null(app_config$unknown_bin_id)) app_config$unknown_bin_id else NULL
    ...
    for (bin_info in bin_contents) {
      ...
      for (item_id in bin_info$item_ids) {
        ...
        # Track score excluding "I don't know" bin
        if (!is.null(unknown_bin_id) && bin_info$bin_id != unknown_bin_id) {
          total_excluding_unknown <- total_excluding_unknown + 1
          if (is_correct) correct_excluding_unknown <- correct_excluding_unknown + 1
        }
    ...
    if (!is.null(unknown_bin_id) && total_excluding_unknown > 0) {
